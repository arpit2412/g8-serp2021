...
FILE_PATH = "The path which contains all the .txt files"
Processed_FILE = "Output Path"
...
for fpathe, dirs, fs in os.walk(FILE_PATH):
    for f in fs:
        if (os.path.splitext(f)[1] == '.txt'):  # Get the .c files only
            ...
            i=i+1
            percent=i/len([lists for lists in os.listdir(FILE_PATH) if os.path.isfile(os.path.join(FILE_PATH, lists))])
            percent="%.2f%%" % (percent * 100)
            print("\rCurrent progress:{}".format(percent), end="")

print("\rTotally, there are " + str(total_processed) + " files.")
